...
MASTER ORCHESTRATOR OF THE ENTIRE RESEARCH AGENT.

This file coordinates every subsystem and defines the
**full lifecycle of an AI-driven research process**.

It does NOT perform heavy intelligence itself.
Instead, it connects specialized modules into a
...
STAGE 0 — Research Planning
    • LLM decomposes the research question into:
        - A research goal
        - Conceptual coverage dimensions

STAGE 1 — Iterative Discovery
    • Generate sub-queries
    • Reuse prior knowledge from vector store
    • Search the web for new sources
    • Summarize evidence
    • Store structured summaries

STAGE 2 — Cross-Source Analysis
    • Detect agreement relationships
    • Score credibility + consensus
    • Detect contradictions
    • Resolve conflicts by removing weaker claims

STAGE 3 — Academic Report Generation
    • Build references
    • Generate title + headings
    • Write full research synthesis
    • Render PDF

STAGE 4 — Self Evaluation
    • Evaluate report against:
        - Research question
...
DESIGN PRINCIPLES
-------------------------------------------------------
• Deterministic structure, probabilistic intelligence
• Evidence-first research
• No duplicate source usage
• Plan-aware evaluation
...
def run_pipeline(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if iteration == 1:
            ...
            trace.log_coverage_refinement(iteration, current_queries)
        ...
        for qkey, qtext in subq_map.items():
            ...
            for h in hits:
                url = h.get("url")
                if url in seen_urls:
                    trace.log_url_skip(url, "already used earlier (vector search)")
                    continue
            ...
            vs_map = {f"VS_{i:02d}": h for i, h in enumerate(hits, 1)}
            ...
            reranked_map = {c["vs_id"]: c for c in reranked}

            trace.log_cross_encoder(
                qkey=qkey,
                reranked={vs_id: data["query_text"] for vs_id, data in reranked_map.items()},
            )
    ...
    agreement_map = detect_agreements([{"id": s["id"], "summary": s["summary"]} for s in summaries])
    ...
    for s in summaries:
        ...
        s["total_score"] = s.get("credibility_score", 0) + s["agreement_score"]

    trace.log_total_scores(summaries)

    conflicts = detect_conflicts([{"id": s["id"], "summary": s["summary"]} for s in summaries])
    trace.log_conflicts(conflicts)

    removals = resolve_conflicts(conflicts, {s["id"]: s["total_score"] for s in summaries})
    trace.log_conflict_resolutions(removals)

    if removals:
        rewrite_plan = {
            sid: {"summary": next(s for s in summaries if s["id"] == sid)["summary"], "remove_claims": claims}
